...
/* A short tag for a person, for a board where the space is a tile.
   ...
   This was initials, and initials do not work here: the staff record holds ONE
   name field, and most of them are a single first name, so nearly everybody
   came out as one letter. B is not a person.

   First three letters of the first word instead. Ben Doyle is BEN, Ana is ANA,
   Jo is JO. Predictable, and it reads as a name rather than as a code. Falls
   back to nothing rather than to a guess, and the caller drops the whole
   phrase when there is nothing to show. */
