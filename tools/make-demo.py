...
def build(src, out, links):
    ...
    for a, b in links.items():                      # keep the nav usable offline
        ...
        # The menu is generated by buildNav from the NAV array in the inlined
        # shared JS, so the links live there as single-quoted strings now,
        # not as pasted href attributes. Rewriting them keeps the demo's own
        # page out of its menu too: buildNav omits the page it stands on by
        # filename, and the demo's filename is the demo one.
        html = html.replace("'%s'" % a, "'%s'" % b)
